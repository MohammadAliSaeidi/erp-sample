...
function Sidebar({
  ...
  /** Overrides `DirectionProvider` / `<html dir>`. When omitted, uses Radix direction context. */
  dir?: "ltr" | "rtl";
}) {
  const dir = useDirection(dirProp);
  const { isMobile, state, openMobile, setOpenMobile } = useSidebar();

  /** Sheet uses physical edges; map logical sidebar `side` for RTL. */
  const sheetSide = dir === "rtl" ? (side === "left" ? "right" : "left") : side;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          // Use inline-start/end so fixed panel matches the flex spacer under `dir=rtl` (side=left → visual right).
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px rtl:translate-x-px rtl:translate-x-px items-center gap-2 overflow-hidden rounded-none px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden group-data-[collapsible=icon]:hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-xs data-[size=sm]:text-xs data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
        className,
      )}
      {...props}
    />
