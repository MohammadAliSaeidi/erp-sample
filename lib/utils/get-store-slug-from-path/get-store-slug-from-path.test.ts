...
describe("getStoreSlugFromPath", () => {
  ...
  describe("valid paths — returns store slug", () => {
    describe("localhost", () => {
      test("without protocol", () => {
        expect(
          getStoreSlugFromPath(`localhost:5173/store/${MOCK_STORE_NAME}/login`),
    ...
    describe("store slug character variations", () => {
      test("slug with hyphens", () => {
        expect(getStoreSlugFromPath(`/store/my-awesome-store/page`)).toBe(
          "my-awesome-store",
        );
      });

      test("slug with underscores", () => {
        expect(getStoreSlugFromPath(`/store/my_store_name/page`)).toBe(
          "my_store_name",
        );
      });

      test("slug with numbers", () => {
        expect(getStoreSlugFromPath(`/store/store123/page`)).toBe("store123");
      });
  ...
  describe("invalid paths — returns null", () => {
    ...
    test("/store/ with no trailing slash (missing sub-page)", () => {
      expect(getStoreSlugFromPath(`/store/${MOCK_STORE_NAME}`)).toBeNull();
    });
    ...
    test("store in hash", () => {
      expect(
        getStoreSlugFromPath(`/products#store/${MOCK_STORE_NAME}/page`),
      ).toBeNull();
    });
