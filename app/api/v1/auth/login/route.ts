...
export const POST = withValidatedBody(loginBodySchema, async (_req, _ctx, body) => {
	...
	const storeRouteIndex = pathnameSegments.findIndex((segment) => segment === "store");
	const storeSlug = storeRouteIndex >= 0 ? pathnameSegments[storeRouteIndex + 1] : undefined;
	...
	if (!store) return NextResponse.json(null, { status: 404, statusText: "Store Not Found" });
	...
	const isValidCredentials = storeUser && (await isHashMatch(body.password, storeUser.password));

	if (!isValidCredentials) {
		return NextResponse.json({}, { status: 401, statusText: "Incorrect username or password" });
	}
